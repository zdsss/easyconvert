...
describe('parseHistoryApi', () => {
  ...
  it('save sends POST', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith('/api/parse-history', expect.objectContaining({ method: 'POST' }));
  });

  it('remove sends DELETE', async () => {
    mockFetch.mockResolvedValue(okResponse(undefined));
    await parseHistoryApi.remove('1');
    expect(mockFetch).toHaveBeenCalledWith('/api/parse-history/1', expect.objectContaining({ method: 'DELETE' }));
  });

  it('clearAll sends DELETE to base', async () => {
    mockFetch.mockResolvedValue(okResponse(undefined));
    await parseHistoryApi.clearAll();
    expect(mockFetch).toHaveBeenCalledWith('/api/parse-history', expect.objectContaining({ method: 'DELETE' }));
  });
