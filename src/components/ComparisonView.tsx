...
export default function ComparisonView({ parsed, annotation }: Props) {
  ...
  return (
    <div className="space-y-5">
      <h3 className="text-xs font-semibold text-text-tertiary uppercase tracking-wide">字段对比</h3>
      ...
      <div className="card overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-surface-secondary">
            <tr>
              <th className="px-4 py-2.5 text-left text-xs font-semibold text-text-tertiary uppercase tracking-wide w-20">字段</th>
              <th className="px-4 py-2.5 text-left text-xs font-semibold text-text-tertiary uppercase tracking-wide">解析结果</th>
              <th className="px-4 py-2.5 text-left text-xs font-semibold text-text-tertiary uppercase tracking-wide">标注数据</th>
              <th className="px-4 py-2.5 text-left text-xs font-semibold text-text-tertiary uppercase tracking-wide w-20">状态</th>
            </tr>
          </thead>
          <tbody>
            {fields.map(field => {
              ...
              return (
                <tr key={field} className={`border-t border-border-light ${style.row}`}>
                  <td className="px-4 py-2.5 font-medium text-text-secondary">{FIELD_LABELS[field] || field}</td>
                  <td className="px-4 py-2.5">{parsedVal != null ? String(parsedVal) : <span className="text-text-tertiary">-</span>}</td>
                  <td className="px-4 py-2.5">{annotationVal != null ? String(annotationVal) : <span className="text-text-tertiary">-</span>}</td>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="card p-4 border-l-2 border-l-brand-400">
            ...
            {parsed.work?.map((w, i) => (
              <div key={i} className="text-sm text-text-secondary mb-1">{w.company} — {w.position}</div>
            )) || <div className="text-sm text-text-tertiary">无</div>}
          </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="card p-4 border-l-2 border-l-brand-400">
            ...
            {parsed.education?.map((e, i) => (
              <div key={i} className="text-sm text-text-secondary mb-1">{e.institution} — {e.degree}</div>
            )) || <div className="text-sm text-text-tertiary">无</div>}
          </div>
          <div className="card p-4 border-l-2 border-l-status-success">
            <div className="text-xs font-semibold text-emerald-600 mb-2">标注数据</div>
