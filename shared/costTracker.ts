import { calculateCost, DEFAULT_MODEL } from './pricing';
...
export class CostTracker {
  ...
  /** Average tokens per call (for estimation fallback) */
  getAvgTokensPerCall(): number {
    const stats = this.getStats();
    return stats.calls > 0 ? Math.round(stats.tokens / stats.calls) : 0;
  }
}
